...
import { useSalesByMonth } from "../api/analytics.api"
import { useAnalyticsFilters } from "../store/analytics-filters.store"
import { formatCompactMXN, formatMXN, MONTH_LABELS } from "../lib/format"
...
export function SalesByMonthChart() {
    ...
    const { data, isLoading } = useSalesByMonth(filters)
    ...
    const chartData = (data ?? []).map((d) => ({
        ...d,
        label: MONTH_LABELS[d.month - 1] ?? String(d.month),
    }))
    ...
    return (
        <Card className="@container/card">
            <CardHeader>
                ...
                <CardDescription>Estacionalidad anual (suma sobre años)</CardDescription>
            </CardHeader>
